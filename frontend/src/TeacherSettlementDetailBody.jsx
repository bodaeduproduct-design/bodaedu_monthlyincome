  ...
  return (
    <div className="teacher-detail-modal">
      ...
      <section className="teacher-detail-block teacher-detail-block--flat">
        {(detail.regular_payments ?? detail.lesson_records ?? []).length === 0 ? (
          <div className="empty-state compact">해당 월 정규 수납이 없습니다.</div>
        ) : (
          <div>
            <div className="teacher-detail-block__header">
              <h4>월별 수업</h4>
              <span>{(detail.regular_monthly_payments ?? []).length}건</span>
            </div>
            {(detail.regular_monthly_payments ?? []).length === 0 ? (
              ...
              <div className="table-wrap">
                <table className="settlement-overview-table">
                  ...
                  <tbody>
                    {(detail.regular_monthly_payments ?? []).map((row) => (
                      <tr key={row.id}>
            ...
            <div className="teacher-detail-subsection teacher-detail-subsection--per-session">
              <div className="teacher-detail-block__header">
                <h4>회차별 수업</h4>
                <span>{(detail.regular_per_session_payments ?? []).length}건</span>
              </div>
              {(detail.regular_per_session_payments ?? []).length === 0 ? (
                ...
                <div className="table-wrap">
                  <table className="settlement-overview-table">
                    ...
                    <tbody>
                      {(detail.regular_per_session_payments ?? []).map((row) => (
                        <tr key={row.id}>
                          <td>{row.student_name ?? `학생#${row.student_id}`}</td>
                          <td>{row.product_name ?? '-'}</td>
      ...
      <section className="teacher-detail-block teacher-detail-block--flat">
        <div className="teacher-detail-block__header teacher-detail-block__header--trial">
          <h4>시범 수업</h4>
          <span>{(detail.trial_lessons ?? []).length}건</span>
        </div>
        {(detail.trial_lessons ?? []).length === 0 ? (
